...
int pre_main(const char *argv)
{
   ...
   Only1Arg = (strcmp(ARGUV[0],"opentyrian") == 0) ? 0 : 1;
...
#ifdef __LINUX__
   sprintf(retro_system_DATA, "%s/tyrian-data\0",RETRO_DIR);
#else
   sprintf(retro_system_DATA, "%s\\tyrian-data\0",RETRO_DIR);
#endif

	custom_data_dir=retro_system_DATA;
